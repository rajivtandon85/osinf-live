...
export function AlertsPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed inset-y-0 right-0 z-50 flex w-full flex-col border-l border-[var(--line)] bg-[var(--bg)] shadow-2xl sm:w-[480px]">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-4">
          <h2 className="text-sm font-semibold text-[var(--text)]/80">Keyword Alerts</h2>
          <button
            onClick={onClose}
            className="rounded-md p-1.5 text-[var(--muted)] transition hover:bg-white/5 hover:text-[var(--text)]/70"
          >
            ✕
          </button>
        </div>
        ...
        {/* Add keyword form */}
        <form onSubmit={handleAdd} className="border-b border-[var(--line)] px-5 py-3">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg bg-white/5 px-3 py-2 text-sm text-[var(--text)]/80 placeholder-[var(--muted)] ring-1 ring-white/10 outline-none transition focus:ring-white/20"
            />
            <button
              type="submit"
            ...
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-medium capitalize transition ${
                tab === t
                  ...
                  : "text-[var(--muted)] hover:text-[var(--text)]/50"
              }`}
            >
              {t}
                ...
                keywords.map((kw) => (
                  ...
                  >
                    <div className="flex flex-col gap-0.5">
                      <span className="text-sm font-medium text-[var(--text)]/70">
                        {kw.keyword}
                      </span>
                    ...
                    <button
                      ...
                      className="ml-3 rounded p-1 text-[var(--muted)] transition hover:bg-red-500/10 hover:text-red-400"
                    >
                      ✕
                    </button>
                  </div>
